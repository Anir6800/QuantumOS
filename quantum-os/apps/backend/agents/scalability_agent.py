...
class ScalabilityAgent(BaseAgent):
    ...
    async def _stream_codegen(self, task_description: str, strategy_context: str, system_prompt: str, strong_retry: bool = False) -> str:
        ...
        async def _run_stream():
            # Resolve best provider+model for coding/scalability tasks dynamically
...
async def main():
    ...
    return await processor.process([{safe_task!r}])
'''
    ...
    def _is_valid_model_output(self, text: str) -> bool:
        return self._is_code_like(text) and "fallback implementation" not in text.lower()
